(($) ->
  $.fn.assetable_uploader = (options) ->
    ...
    defaults =
      ...
      url: null
      uploader_id: null
      uploadProgress: (up, file, assetable_uploader) ->
        $("li#" + file.id + " .progress .progress-bar", assetable_uploader).width file.percent + "%"
      filesAdded: (up, files, assetable_uploader) ->
        $.each files, (i, file) ->
      ...
      fileUploaded: (up, file, info, assetable_uploader) ->
        eval(info.response)
    ...
    bind_uploader = ->
      ...
      uploader.bind "FilesAdded", (up, files) ->
        assetable_uploader.options.filesAdded(up, files, assetable_uploader)


      # Update the progress bars
      uploader.bind "UploadProgress", (up, file) ->
        assetable_uploader.options.uploadProgress(up, file, assetable_uploader)


      # Listen for upload complete
      uploader.bind "FileUploaded", (up, file, info) ->
        assetable_uploader.options.fileUploaded(up, file, info, assetable_uploader)
